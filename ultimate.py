...
def run_createtabletem():
    ...
    try:
        ...
        log.info("🖼️ Створено colored.png успішно.")
        if result.stdout:
        ...
        log.error("⏱️ createtabletem.py завис і був зупинений (timeout).")
        return False
...
def send_image_to_channels(post_text: str, schedule_txt: str):
    ...
    if test_mode:
        ...
        log.info(f"🧪 TEST_MODE активний. Буде надіслано в тестові канали: {channels}")
    else:
    ...
    image_path = Path("colored.png")
    if not image_path.exists():
        log.warning("❌ Файл colored.png не знайдено.")
        return

    for ch_id in channels:
        try:
            bot.send_photo(chat_id=ch_id, photo=image_path.open("rb"), caption=caption)
    ...
    while True:
        try:
            ...
            for pid, text in sorted(posts.items()):
                ...
                json_file = save_schedule(schedule, date_obj)
